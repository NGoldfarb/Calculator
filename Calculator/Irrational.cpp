...
Number* Irrational::add(Number* a) {
	if (a->getType() == type && value == a->getCharValue()) {
		if (value == a->getCharValue()
				&& exp->getValue() == a->getExp()->getValue()) {
			if (coef->getType() == "Basic"
					&& a->getCoef()->getType() == "Basic") {
			...
			if (coef->getType() == "Fraction"
					&& a->getCoef()->getType() == "Fraction") {
...
Number* Irrational::expo(Number* a) {
	Number* b = exp->multiply(a);
	Number* c = new Irrational(value, coef, b);
	return c;
}
